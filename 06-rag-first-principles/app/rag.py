...
def _embed(text: str, dimensions: int = 256) -> list[float]:
    # Educational deterministic embedding. Replace with a benchmarked model in production.
...
class VectorIndex:
    ...
    def search(self, query: str, k: int = 5, tenant_id: str | None = None) -> list[Evidence]:
        if k <= 0:
            return []
        q = _embed(query)
        candidates = [
            ...
            if tenant_id is None or chunk.tenant_id == tenant_id
        ]
