...
def main():
    ...
    start,end=map(int,args.modules.split('-')); os.environ.setdefault('MOCK_LLM','true'); os.environ.setdefault('OPENAI_API_KEY',''); os.environ.setdefault('ANTHROPIC_API_KEY',''); failures=[]; total=0
    for n,p in notebooks(range(start,end+1)):
        total+=1; print(f'EXECUTE M{n:02d}: {p.relative_to(ROOT)}')
        try:
            nb=nbformat.read(p,as_version=4); module_root=p.parent.parent; existing=os.environ.get('PYTHONPATH',''); os.environ['PYTHONPATH']=str(module_root)+(os.pathsep+existing if existing else '')
            NotebookClient(nb,timeout=args.timeout,kernel_name='python3',resources={'metadata':{'path':str(p.parent)}}).execute(); print(f'PASS M{n:02d}: {p.name}')
